...
public class NewBeeMallCloudAdminUserController {
    ...
    public Result profile(@TokenToAdminUser AdminUserToken adminUser, HttpServletRequest request) {
    ...
    public Result passwordUpdate(@RequestBody @Valid UpdateAdminPasswordParam adminPasswordParam, @TokenToAdminUser AdminUserToken adminUser) {
        logger.info("adminUser:{}", adminUser.toString());
        if (adminUserService.updatePassword(adminUser.getAdminUserId(), adminPasswordParam.getOriginalPassword(), adminPasswordParam.getNewPassword(), adminUser.getToken())) {
